...
namespace LethalIntelligence.Patches
{
    ...
    public class MaskedAIRevamp : NetworkBehaviour
    {
        ...
        int savedFocusInt;
        int savedActivityInt;
        int countOfIdle = 0;

        private void EnterIdleMode()
        {
            //if a masked's path is blocked, they should enter idle mode, and do random things, until the path is no longer blocked.
            if (savedPath == null)
            {
                savedPath = agent.path;
                savedDestination = agent.destination;
                savedFocusInt = (int)maskedFocus;
                savedActivityInt = (int)maskedActivity;
            }
            if(countOfIdle > 3)
            {
                countOfIdle = 0;
            ...
            int doorMask = 1 << NavMesh.GetAreaFromName("Door");
            NavMeshHit doorHit;
            if (!agent.SamplePathPosition(NavMesh.AllAreas, 1.0F, out doorHit))
            {
                if ((doorHit.mask & doorMask) != 0)
                {
                    //path blocked..
                    Plugin.mls.LogError("PathBlocked=" + doorHit.mask + "||" + doorMask);
                }
            }
            //Plugin.mls.LogError("PathInvalid=" + !agent.CalculatePath(savedDestination, savedPath));
            //Plugin.mls.LogError("Path.Status=" + agent.pathStatus);
            if (!agent.CalculatePath(savedDestination, savedPath) || agent.pathStatus == NavMeshPathStatus.PathInvalid)
            {
                maskedFocusInt.Value = (int)Focus.None;
                maskedActivityInt.Value = (int)Activity.Idle;
                maskedGoal = "Path Blocked, being Idle!";
                countOfIdle++;
                maskedEnemy.LookAndRunRandomly();
            }
            else
            {
                maskedFocusInt.Value = savedFocusInt;
                maskedActivityInt.Value = savedActivityInt;
                agent.path = savedPath;

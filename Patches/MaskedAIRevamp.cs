...
//TimeSinceTeleporting += updateFrequency;
        ...
        public LethalNetworkVariable<float> interestInPlayers = new LethalNetworkVariable<float>("interestInPlayers");
        int maskedOriginalHP = -5;
        int killedPlayerCount = 0;
        ...
        private void DetectAndSelectRandomPlayer()
        {
            ...
            if (players != null && maskedOriginalHP == -5) //this means this is the first time a masked has seen a player.
            {
                maskedOriginalHP = __instance.enemyHP; //should only occur once..
            }
            if (maskedFocus == Focus.Player || (maskedFocus == Focus.None && maskedActivity == Activity.RandomPlayer) || maskedEnemy.inSpecialAnimation)
            {
                //dont use this to find or focus on players as you are already found a player or are in a special animation
                return;
            }
            maskedGoal = "Detecting Players";
            //triggers...
            float watchingTrigger, seenTrigger, attackedTrigger, killedTrigger;
            //any players ARE or ARENT watching masked.
            PlayerControllerB[] scripts = StartOfRound.Instance.allPlayerScripts;
            if (scripts.Any(p => p.HasLineOfSightToPosition(maskedEnemy.transform.position)))
            {//players are watching masked
                watchingTrigger = 0.2f;
            }
            else
            {
                watchingTrigger = -0.2f;
            }
            //masked has seen any players
            if(players.Length > 0)
            {
                seenTrigger = 0.2f * players.Length;
            }
            else
            {
                seenTrigger = -0.2f * scripts.Length;
            }
            //any player has attacked masked.
            if(__instance.enemyHP != maskedOriginalHP)
            {
                maskedOriginalHP = __instance.enemyHP;
                attackedTrigger = 90f;
            }
            else
            {
            ...
            //masked has killed player
            if(maskedEnemy.playersKilled.Count() > killedPlayerCount)
            {
                killedPlayerCount++;
                killedTrigger = -90f;
            }
            ...
            switch(maskedPersonality)
            {
                case Personality.Aggressive:
                    float temp = (watchingTrigger + seenTrigger + attackedTrigger + killedTrigger);
                    if(temp > 0f)
                    {
                        temp = temp * 5;
                    }
                    interestInPlayers.Value += temp;
                    break;
                case Personality.Cunning:
                    interestInPlayers.Value += ((watchingTrigger * 2) + (seenTrigger / 2) + (attackedTrigger * 4 ) + (killedTrigger * 2));
                    break;
                case Personality.Stealthy:
                    interestInPlayers.Value += ((watchingTrigger / 4) + (seenTrigger / 4) + attackedTrigger + killedTrigger);
                    break;
                case Personality.Insane:
                    interestInPlayers.Value += ((watchingTrigger / 2) + (seenTrigger * 2) + (attackedTrigger * 10) + (killedTrigger / 2));
                    break;
                case Personality.Deceiving:
                    interestInPlayers.Value -= ((watchingTrigger / 2) + (seenTrigger / 2) + attackedTrigger + killedTrigger);
                    break;
            }

            if(interestInPlayers.Value > 100f && players.Length > 0)
            {
                int random = UnityEngine.Random.RandomRangeInt(0, players.Length);
                mustChangeActivity = false;
                mustChangeFocus = false;
                maskedFocus = Focus.None;
        ...
        public void DetectEnemy()
        {
            ...
            if (interestInPlayers.Value < 100f)
            {
                return; //shouldent be doing any of this until interested enough.
            }

...
namespace LethalIntelligence.Patches
{
    ...
    public class MaskedAIRevamp : NetworkBehaviour
    {
        ...
        public LethalNetworkVariable<int> maskedCurrentPatience = new LethalNetworkVariable<int>("maskedCurrentPatience");
        public int maskedMaxPatience;
        ...
        public void FixedUpdate()
        {
            if (Plugin.imperiumFound)
            {
                try
                {
                ImperiumPatches.maskedVisualization(maskedPersonality, maskedActivity, maskedFocus,maskedCurrentPatience.Value);
            }
        ...
        //private void changePatience(PlayerControllerB[] players)
        private void changePatience()
        {
            PlayerControllerB[] players2 = maskedEnemy.GetAllPlayersInLineOfSight(80f, 60, null, 10, -1);
            if (players2 == null)
            {
                maskedCurrentPatience.Value = maskedCurrentPatience.Value + 5;
                return;
            }
            foreach (PlayerControllerB player in players2)
            {
                Plugin.mls.LogError("PatienceChange!");
                Plugin.mls.LogWarning("PatienceAtStart=" + maskedCurrentPatience.Value);

                //player watching masked?
                Plugin.mls.LogError("PlayerLookingAtMasked:" + player.HasLineOfSightToPosition(maskedEnemy.transform.position, 80f, 60));
                if (player.HasLineOfSightToPosition(maskedEnemy.transform.position, 80f, 60))
                {
                    //looking at masked = + 1 (if deceiving, +2)
                    if(maskedPersonality == Personality.Deceiving)
                    {
                        maskedCurrentPatience.Value = maskedCurrentPatience.Value + 1;
                    }
                    else
                    {
                        maskedCurrentPatience.Value = maskedCurrentPatience.Value + 1;
                ...
                {
                    //looking away from masked = -1 (if deceiving, -3)
                    if (maskedPersonality == Personality.Deceiving)
                    {
                        maskedCurrentPatience.Value = maskedCurrentPatience.Value - 3;
                    }
                    else
                    {
                        maskedCurrentPatience.Value = maskedCurrentPatience.Value - 2;
                    }
                }

                //close to masked?
                Plugin.mls.LogError("PlayerDistanceToMasked:" + Vector3.Distance(player.transform.position,maskedEnemy.transform.position));

                if(Vector3.Distance(player.transform.position, maskedEnemy.transform.position)<3f)
                {
                    maskedCurrentPatience.Value = maskedCurrentPatience.Value - 5;
                }
                else
                ...
                if (player.currentlyHeldObjectServer == null) {
                    Plugin.mls.LogError("CurrentlyHeldObjectServer:" + "null"); 
                } 
                else
                {
                    if(player.currentlyHeldObjectServer is ShotgunItem)
                    {
                        //holding ranged weapon = -5
                        maskedCurrentPatience.Value = maskedCurrentPatience.Value - 5;
                        Plugin.mls.LogError("CurrentlyHeldObjectServer:" + player.currentlyHeldObjectServer.ToString());
                    }
                    else if (player.currentlyHeldObjectServer is KnifeItem || player.currentlyHeldObjectServer is Shovel)
                    {
                    ...
                    {
                        Plugin.mls.LogError("CurrentlyHeldObjectServer:" + "not a weapon...lol");
                    }
                }


                Plugin.mls.LogWarning("PatienceAtEnd=" + maskedCurrentPatience.Value);
            }
        }
